...
exports.search = async (req, res) => {
  const { query, curso, semestre } = req.query;
  ...
  try {
    ...
    if (query) {
      sql += ` AND (d.nome ILIKE $${paramIndex} OR d.curso ILIKE $${paramIndex})`;
      params.push(`%${query}%`);
      paramIndex++;
    }

    if (curso) {
      sql += ` AND d.curso ILIKE $${paramIndex}`;
      params.push(`%${curso}%`);
      paramIndex++;
    }

    if (semestre) {
      sql += ` AND d.semestre = $${paramIndex}`;
